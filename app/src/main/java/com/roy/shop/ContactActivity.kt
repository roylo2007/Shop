...
class ContactActivity : AppCompatActivity() {
    ...
    private fun readContacts() {
        val currsor = contentResolver.query(
            ContactsContract.Contacts.CONTENT_URI,
            null, null, null, null
        )
        while (currsor.moveToNext()) {
            val name = currsor.getString(currsor.getColumnIndex(ContactsContract.Contacts.DISPLAY_NAME))
            Log.d(TAG, "onCreated: ${name}");
        }
    }
}
